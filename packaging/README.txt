...
windows
- run the client with `OG2.exe`; it starts the launcher so client updates can be checked before the game opens
- the actual game binary is `OG2.Game.exe`; launch `OG2.exe` for normal play so updates are checked

linux/macOS
- run the client with `./OG2` when app files are executable, or `sh run-client.sh`; it starts the launcher so client updates can be checked before the game opens
