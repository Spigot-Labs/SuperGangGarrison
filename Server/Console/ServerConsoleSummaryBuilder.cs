    ...
    Func<string>? demoStatusLineGetter = null)
{
    public void AddStatusSummary(List<string> lines)
    {
        ...
        lines.Add(
            $"[server] snapshots | frame={snapshotMetrics.Frame} | clients={snapshotMetrics.ClientCount} | intervalMs={snapshotMetrics.BroadcastIntervalMilliseconds:F1}/{snapshotMetrics.BroadcastTargetIntervalMilliseconds:F1} | frameDelta={snapshotMetrics.SnapshotFrameDelta} | overMs={snapshotMetrics.BroadcastIntervalOverrunMilliseconds:F1} | sharedBuildMs={snapshotMetrics.SharedCaptureMilliseconds:F2} | clientBuildMs={snapshotMetrics.PerClientMilliseconds:F2} | totalMs={snapshotMetrics.TotalMilliseconds:F2} | allocKB={snapshotMetrics.TotalAllocatedBytes / 1024d:F1} (shared={snapshotMetrics.SharedCaptureAllocatedBytes / 1024d:F1}, client={snapshotMetrics.PerClientAllocatedBytes / 1024d:F1})");
        lines.Add(
